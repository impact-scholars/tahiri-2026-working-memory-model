# Run_Grid_Trials.py
"""
Grid search over (nmda_scale, gaba_scale, cue_rate, Jp, distractor_rate, seed).
"""
...
FULL_GRID = dict(
    nmda_scale       = [1.10, 1.00, 0.95, 0.90,0.8,0.6,0.5,0.4,0.3,0.2,0.1,0],
    gaba_scale       = [1.10, 1.00, 0.95, 0.90,0.8,0.6,0.5,0.4,0.3,0.2,0.1,0],
    cue_rate         = [200],
    Jp               = [1.75,1.78,1.8,1.82,1.84,1.85,1.86,1.88,1.9],
...
QUICK_GRID = dict(
    ...
    cue_rate         = [200],
    Jp               = [1.84],
    distractor_rates = [200],   # no-distractor vs strong distractor
    seeds            = [42],
)
...
def run_grid(grid, base_cfg, csv_path="results/grid_results.csv",
             resume=False):
    """
...
def main():
    ...
    # CLI overrides for distractor_rates and seeds
    if args.distractor_rates is not None:
        grid = dict(grid)                          # don't mutate module-level dict
    ...
    print(f"Grid size : {total_combos} simulations")
    ...
    df = run_grid(grid, base_cfg, csv_path=args.csv, resume=args.resume)
    ...
    # Heatmap: no-distractor trials
    plot_heatmap(df, metric="r1_delay", cue_r=200, jp=1.84,
                 distractor_rate=0, aggregate_seeds=True)

    # Heatmap: strong-distractor trials
    if 200 in grid["distractor_rates"]:
        plot_heatmap(df, metric="r1_delay", cue_r=200, jp=1.84,
                     distractor_rate=200, aggregate_seeds=True)

    # Distractor curve for the control point
    plot_distractor_curve(df, nmda_s=1.00, gaba_s=1.00,
                          cue_r=200, jp=1.84, metric="r1_delay")

    # Seed-aggregated DataFrame (optional, for downstream analysis)
